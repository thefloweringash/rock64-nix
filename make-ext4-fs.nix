...
let installSpec = deriv: pkgs.runCommand "fs-spec-${deriv.name}" {} ''
    # XXX: This explodes in exciting ways if anything in /nix/store has a space in it.
    find ${deriv} -printf '%y %p /%P %m\n'| while read -r type from to perms; do
...
in pkgs.stdenv.mkDerivation {
  ...
  installSpecs = map installSpec installPaths;
